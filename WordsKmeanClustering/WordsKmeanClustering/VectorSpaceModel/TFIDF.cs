        ...
        private static double[][] TransformToTFIDFVectors(List<List<string>> stemmedDocs, Dictionary<string, double> vocabularyIDF)
        {
            ...
            foreach (var doc in stemmedDocs)
            {
                ...
                foreach (var vocab in vocabularyIDF)
                {
                    ...
                    int i = 0;
                     SqlCommand c = new SqlCommand("INSERT INTO WordsWeight (Term,TF,IDF,Weight) VALUES(@t,@f,@i,@w)", con);
                       // c.Parameters.AddWithValue("@d", i++);
                     c.Parameters.AddWithValue("@t",vocab.Key);
                     c.Parameters.AddWithValue("@f", tf);
                     c.Parameters.AddWithValue("@i", vocab.Value);
                     c.Parameters.AddWithValue("@w", tfidf);
