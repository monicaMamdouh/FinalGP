              ...
              SqlConnection con = new SqlConnection("Data Source=DESKTOP-FQEB1DN;Initial Catalog=UserInterestClassification ;Integrated Security=True");
               ...
               foreach (DataRow row in dataTable.Rows)
               {
                   ...
                   if (!index.ContainsKey(Term))    //check if the dictionary Not contain the same term 
                   {
                       ...
                       idx.Add(new InvertedItem(category, pos, freq));
                       index.Add(Term, idx);
                   ...
                   else
                   {
                       ...
                       t = index[Term];
                       t.Add(new InvertedItem(category, pos, freq));
                       idx.AddRange(t);
               ...
               foreach (String key in keyList)
               {
                   ...
                   try
                   {
                       SqlCommand c = new SqlCommand("INSERT INTO InterestInvertedIndex (Term,category,frequency,position) VALUES(@t,@g,@f,@p)", con);
